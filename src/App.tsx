import type { FileInfo, Headers } from './types'
import { LinkOutlined } from '@ant-design/icons'
import { Button, message, Space, Spin, Switch, Typography } from 'antd'
import React, { Suspense, useRef, useState } from 'react'
import { v4 as uuidv4 } from 'uuid'
import { FileDownloader } from './logics/FileDownloader'
import { FileExplorer } from './logics/FileExplorer'
import './App.css'
...
const App: React.FC = () => {
  ...
  const login = async () => {
    setLoading(true)
    try {
      const response = await fetch('https://www.123pan.com/b/api/user/sign_in', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...headers,
